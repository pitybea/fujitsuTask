...
public:
	...
	inline float dot_(const INT64 tig1, const INT64 tig2, const INT64 tig4, const INT64 tig8);
...
inline float FilterTIG::dot_(const INT64 tig1, const INT64 tig2, const INT64 tig4, const INT64 tig8)
{
...
template<class T>
vector<vector<T> > VisibleMatrix(const Mat& mat)
{
	vector<vector<T> > rslt(mat.rows,vector<T>(mat.cols));
	for (int i = 0; i < mat.rows; i++)
	{
		for (int j = 0; j < mat.cols; j++)
		{
			rslt[i][j]=mat.at<T>(i,j);
		}
	}
