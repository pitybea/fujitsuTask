...
void Objectness::predictBBoxSI(CMat &img3u, ValStructVec<float, Vec4i> &valBoxes, vecI &sz, int NUM_WIN_PSZ, bool fast)
{
	...
	for (int ir = numSz - 1; ir >= 0; ir--){
		...
		auto resize_s=Size(cvRound(_W*imgW*1.0/width), cvRound(_W*imgH*1.0/height));
		resize(img3u, im3u, resize_s);
		gradientMag(im3u, mag1u);
		
		auto vsth1=VisibleMatrix<byte>(mag1u);
		matchCost1f = _tigF.matchTemplate(mag1u);

		auto vsth2=VisibleMatrix<float>(matchCost1f);
...
#pragma omp parallel for
	...
	auto vst3=VisibleMatrix<float>(wMat);
...
void Objectness::boundboxesforImgs(const vector<string>& fileNames,vector<vector<Vec4i>> &boxesTests, int numDetPerSize)
{
	_chdir("D:\\ZPWang\\objectness\\x64");
	loadTrainedModel();
	boxesTests.resize(fileNames.size());
	vector<Mat> imgs(fileNames.size());

	_chdir("D:\\ZPWang\\vanish\\dataset\\ny");
	for(size_t i=0;i<fileNames.size();i++)
	{
		imgs[i]=imread(fileNames[i]+".jpg");
		boxesTests.reserve(20000);
	}
	vector<ValStructVec<float, Vec4i>> scoreboxesTests(fileNames.size());
#pragma omp parallel for
	for (int i = 0; i < imgs.size(); i++)
	{
		cout<<fileNames[i]<<" ";
		getObjBndBoxes(imgs[i], scoreboxesTests[i], numDetPerSize);
	}
#pragma omp parallel for
	for (int i = 0; i < imgs.size(); i++)
	{
		
		ValStructVec<float, Vec4i> &boxes = scoreboxesTests[i];
		
		boxesTests[i].resize(scoreboxesTests[i].size());
		int num=0;
		for (int j = 0; j < boxesTests[i].size(); j++)
		{
			boxesTests[i][j] = scoreboxesTests[i][j];
			//if(j<10)
			
			if(num<20)
			{
				int xmin(boxesTests[i][j][0]),ymin(boxesTests[i][j][1]),xmax(boxesTests[i][j][2]),ymax(boxesTests[i][j][3]);
				if(((xmax-xmin)<100)&&((ymax-ymin)<120))
				{
					rectangle(imgs[i],Point(xmin,ymin),
					Point(xmax,ymax),Scalar(rand()/255,rand()/255,rand()/255),2,i,0);
					++num;
				}
			}		
		}
		imwrite(fileNames[i]+"_sa.jpg",imgs[i]);
		cout<<fileNames[i]<<" ";
	}
...
#pragma omp parallel for
	for (int i = 0; i < TestNum; i++){
		auto sth=format(_S(_voc.imgPathW), _S(_voc.testSet[i]));
		imgs3u[i] = imread(sth);
		boxesTests[i].reserve(10000);
	}
